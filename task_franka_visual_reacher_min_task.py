...
def parse_args():
    parser = argparse.ArgumentParser(description='Local Visual Franka Reacher')
    # environment
    parser.add_argument('--setup', default='Visual-Franka-Min-Time')
    parser.add_argument('--env', default='Visual-Franka-Min-Time', type=str)
    # parser.add_argument('--franka_ip', default='129.128.159.210', type=str)
...
def main():
    ...
    env = FrankaPanda_Visual_Min_Reacher(episode_length = 150, 
    ...
    image_to_show = np.transpose(image, [2, 1, 0])[:,:,0:3]
    print(image_to_show.shape, prop)
    cv2.imshow('s', np.array(image_to_show, dtype=np.uint8))
    cv2.waitKey(0) 
    input('go?')
    ...
    for step in range(args.env_steps + args.init_steps):
        ...
        next_image, next_prop, reward, done = env.step(action)
